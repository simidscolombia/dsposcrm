...
router.all('/init-tables', async (req, res) => {
    try {
        ...
        try {
            ...
            await db.query(`ALTER TABLE crm_products ADD COLUMN IF NOT EXISTS system_type VARCHAR(50) DEFAULT 'All'`);
        ...
        if (parseInt(checkProducts.rows[0].count) === 0) {
            await db.query(`
                INSERT INTO crm_products (name, description, price, category, stock, image_url, system_type) VALUES
                ('PC All-in-One Industrial', 'Pantalla táctil 15", Procesador Intel, 8GB RAM', 1850000, 'hardware', 10, 'https://images.unsplash.com/photo-1591485423007-765bde4139ef?w=400', 'Combo'),
                ('Impresora Térmica 80mm', 'Impresora de alta velocidad con corte automático (USB/LAN)', 450000, 'hardware', 25, 'https://images.unsplash.com/photo-1612815154858-60aa4c59eaa6?w=400', 'Mix'),
                ('Cajón Monedero Reforzado', 'Apertura automática mediante impresora, 5 puestos de billetes', 220000, 'hardware', 15, 'https://images.unsplash.com/photo-1556740758-90de374c12ad?w=400', 'Mix'),
                ('Lector de Código de Barras 1D/2D', 'Lectura de códigos QR y lineales, base incluida', 180000, 'hardware', 30, 'https://images.unsplash.com/photo-1601598851547-4302969d0614?w=400', 'Mix'),
                ('Software Discovery POS PRO', 'Licencia vitalicia, facturación electrónica, control de inventario', 1200000, 'software', 999, 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=400', 'Software'),
                ('Soporte Técnico 24/7 (1 Mes)', 'Asistencia remota ilimitada y actualizaciones gratuitas', 50000, 'servicios', 999, 'https://images.unsplash.com/photo-1521791136366-3d3950ef770b?w=400', 'All');
            `);
