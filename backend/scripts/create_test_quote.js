...
try {
    ...
    const leadRes = await client.query(`
        INSERT INTO leads (name, whatsapp, city, pipeline_stage, source, status)
        VALUES ('Cliente Prueba #25', '573001234567', 'Medellín', 'quoted', 'web', 'new')
        RETURNING id
    `);
    const leadId = leadRes.rows[0].id;

    // 2. Create the quote with ID 25 to match user's URL
    await client.query(`
